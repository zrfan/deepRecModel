...
class BiFFMModel(object):
    ...
    def biffm_model_fn(self, feature, labels, mode):
        ...
        bias = weights["bias"]
        interaction_weights_shared = weights["interaction_weights_shared"]   # [emb_size, emb_size]
        ...
        feature_embeddings = tf.nn.embedding_lookup(all_embeddings, feature_idx)   # [batch, len, emb_size]
        ...
        for i in range(1, feature_size+1):
            for j in range(i+1, feature_size+1):
                emb_i, emb_j = all_embeddings[i, :], all_embeddings[j, :]
